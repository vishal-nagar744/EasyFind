...
const RestaurnatDetails = () => {
    ...
    useEffect(() => {
        const fetchRestaurantDetails = async () => {
            try {
                const Restaurant = await request(`/Restaurant/find/${id}`, 'GET')
                setRestaurant(Restaurant)
                setIsBookmarked(Restaurant.bookmarkedUsers.includes(user._id))
            } catch (error) {
                console.log(error);
            }
        }
    ...
    const handleDelete = async () => {
        try {
            await request(`/Restaurant/${id}`, "DELETE", { 'Authorization': `Bearer ${token}` })
    ...
    const handleBookmark = async () => {
        try {
            await request(`/Restaurant/bookmark/${id}`, 'PUT', { Authorization: `Bearer ${token}` })
    ...
    return (
        <div className={classes.container}>
            <h3 style={{ textAlign: 'center', marginBottom: '2.5rem', fontSize: '32px', marginTop: '-2.5rem' }}>Restaurant Details</h3>
            <div className={classes.wrapper}>
                ...
                <div className={classes.right}>
                    <div className={classes.top}>
                        <h2 >Title: {Restaurant?.title}</h2>
                        {Restaurant?.currentOwner?._id.toString() === user._id &&
                            (
                    ...
                    {/* <div className={classes.location}>Daily Tifin Price : <span>{Restaurant?.location}</span></div> */}
                    <div className={classes.price}>Monthly Tifin Price: <span>INR {Restaurant?.price}</span></div>
                     <div className={classes.passengers}> Daily Tifin Price : <span>{Restaurant?.maxPassengers}</span></div> 
                    <div className={classes.descContainer}>Location: <p>{Restaurant?.desc}</p></div>
                    <div className={classes.metersLong}>  How Meters long: <span>{Restaurant?.metersLong}</span></div>
                    <div className={classes.ownerDetails}>
                        <span>Owner name: {Restaurant?.currentOwner?.username}</span>
                        {Restaurant?.currentOwner?._id.toString() !== user._id && (
                            <div className={classes.contactBookmarkControls}>
                                <button onClick={() => setShowModal(prev => !prev)}>Contact me on: {Restaurant?.currentOwner?.email}</button>
                        ...
                        <div className={classes.contactForm} onClick={handleCloseForm}>
                            <div className={classes.contactFormWrapper} onClick={(e) => e.stopPropagation()}>
                                ...
                                <form onSubmit={handleContactOwner} ref={formRef}>
                                    ...
                                    <input value={Restaurant?.currentOwner?.email} type="email" placeholder='Owner email' name="to_email" />
...
export default RestaurnatDetails