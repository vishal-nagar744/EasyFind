...
// TODO FETCH TYPE OF PROPERTIES. EX: {ROOM: 34, HOSTEL: 23}
propertyController.get('/find/types', async (req, res) => {
    try {
        const RoomType = await Property.countDocuments({ type: 'Room' })
        const HostelType = await Property.countDocuments({ type: 'Hostel' })
        const FlatType = await Property.countDocuments({ type: 'Flat' })

        return res.status(200).json({ Room: RoomType, Hostel: HostelType, Flat: FlatType })
    } catch (error) {
        return res.status(500).json(error)
    }
})
